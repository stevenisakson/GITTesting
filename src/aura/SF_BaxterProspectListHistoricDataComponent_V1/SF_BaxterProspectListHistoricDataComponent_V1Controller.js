({
	doInit : function(component, event, helper) {
		var action = component.get("c.getAttachmentListProspect"); 
        var action1 = component.get("c.getCommunityDetails"); // get community name details
        action.setCallback(this, function(a){            
            if (a.getState() === "SUCCESS"){
                var arrayProspect = a.getReturnValue();
                arrayProspect.sort(function(a, b){
                    var dateA=new Date(a.UploadDate), dateB=new Date(b.UploadDate)
                    //return dateA-dateB //sort by date ascending
                    return dateB-dateA //sort by date descending
                })
            	component.set('v.prospectList',arrayProspect);                    
